...
async function fetchFNG(){
  try{
    var r = await fetch('/api/fng',{cache:'no-store'}); if(r.ok){ return await r.json(); }
  }catch(e){}
  try{
    var r2 = await fetch('https://api.alternative.me/fng/?limit=1&format=json&date_format=iso',{cache:'no-store'});
    if(!r2.ok) throw 0; return await r2.json();
  }catch(e){ return null; }
}
/* Binance 글로벌 롱/숏 (BTCUSDT 1h) */
async function fetchLongShort(){
  var proxy = '/api/binance/globalLongShortAccountRatio?symbol=BTCUSDT&period=1h&limit=1';
  try{
    var r = await fetch(proxy,{cache:'no-store'}); if(r.ok) return await r.json();
  }catch(e){}
  try{
    var url='https://fapi.binance.com/futures/data/globalLongShortAccountRatio?symbol=BTCUSDT&period=1h&limit=1';
    var r2 = await fetch(url,{cache:'no-store'}); if(!r2.ok) throw 0; return await r2.json();
  }catch(e){ return null; }
}
...
function buildRowHTML(c){
  ...
  var cap = abbrUSD(c.market_cap);
  var vol = abbrUSD(c.total_volume);
  ...
  '<tr class="row">'+
    ...
    '<td class="text-right" data-label="시가총액">'+ cap +'</td>'+
    '<td class="text-right" data-label="거래량">'+ vol +'</td>'+
...
async function renderHeaderMinis(markets){
  try{
    ...
    if (btc && btc.market_cap && btc.current_price && btc.sparkline_in_7d && Array.isArray(btc.sparkline_in_7d.price)) {
      ...
      var elB = $("#mini-btc"); if (elB && capsB.length>1) elB.innerHTML = sparklineSVGFilled(capsB,420,90);
    }
  }catch(e){}
  try{
    ...
    if (usdt && usdt.market_cap && usdt.current_price && usdt.sparkline_in_7d && Array.isArray(usdt.sparkline_in_7d.price)) {
      ...
      var elU = $("#mini-usdt"); if (elU && capsU.length>1) elU.innerHTML = sparklineSVGFilled(capsU,420,90);
    }
  }catch(e){}
}

/* --- KPIs / Lists --- */
function renderKPIs(markets, global, fng, ls){
  var btc = markets.find(function(x){return x.id==="bitcoin";});
  var usdt = markets.find(function(x){return x.id==="tether";});
  ...
  safeSetHTML("#kpi-btc-mcap", btcCap ? "$"+abbrUSD(btcCap) : "-");
  safeSetHTML("#kpi-usdt-mcap", usdtCap ? "$"+abbrUSD(usdtCap) : "-");
  safeSetHTML("#kpi-dominance", dom!=null ? fmtNum(dom,2)+"%" : "-");

  // Fear & Greed
  if(fng && fng.data && fng.data[0]){
    var v = fng.data[0];
    safeSetHTML("#kpi-fng", v.value + " / " + (v.value_classification||""));
    safeSetHTML("#kpi-fng-sub", "Alternative.me · " + (v.timestamp || v.time_until_update || ""));
  }else{
    safeSetHTML("#kpi-fng","연동 예정"); safeSetHTML("#kpi-fng-sub","Alternative.me API");
  }

  // Long/Short (Binance)
  if(ls && ls.length && ls[0].longShortRatio){
    var r = Number(ls[0].longShortRatio);
    var longPct = r/(1+r)*100, shortPct = 100-longPct;
    safeSetHTML("#kpi-longshort", longPct.toFixed(1) + "% / " + shortPct.toFixed(1) + "%");
    safeSetHTML("#kpi-longshort-sub", "BTCUSDT · ratio "+r.toFixed(2));
  }else if(ls && ls.data && ls.data.length && ls.data[0].longShortRatio){
    var r2 = Number(ls.data[0].longShortRatio);
    var L = r2/(1+r2)*100, S = 100-L;
    safeSetHTML("#kpi-longshort", L.toFixed(1)+"% / "+S.toFixed(1)+"%");
    safeSetHTML("#kpi-longshort-sub", "BTCUSDT · ratio "+r2.toFixed(2));
  }else{
    safeSetHTML("#kpi-longshort","연동 예정");
    safeSetHTML("#kpi-longshort-sub","Binance 공개지표");
  }
}
...
function renderRightLists(markets){
  ...
  // TOP 10 volume
  var vol = markets.slice().sort(function(a,b){ return b.total_volume-a.total_volume; }).slice(0,10);
  safeSetHTML("#list-volume", vol.map(function(c,i){
    var sym=(c.symbol||"").toUpperCase();
    return '<div class="row"><div class="rank">'+(i+1)+'.</div><div class="sym">'+sym+
           '</div><div class="price">$'+fmtPrice(c.current_price)+'</div><div class="value">'+abbrUSD(c.total_volume)+'</div></div>';
  }).join(""));
}
