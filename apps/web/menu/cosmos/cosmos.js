/* ===== COSMOS JS – table click → chart.html, mobile polish, stars, reflow ===== */
...
function renderTableSlice(rows){const tbody=ensureTbody(); if(!tbody) return; const s=(state.page-1)*state.perPage,e=s+state.perPage; const slice=rows.slice(s,e); setHTML(tbody, slice.map(buildRowHTML).join("") || `<tr><td colspan="9" class="text-center">데이터 없음</td></tr>`); forceReflow(tbody);}
function applySortFilter(){
  ...
  if(sel){ sel.innerHTML=Array.from({length:totalPages},(_,i)=>{const s=i*state.perPage+1,e=Math.min(state.filtered.length,(i+1)*state.perPage);return `<option value="${i+1}">${s}~${e}</option>`}).join(""); if(state.page>totalPages)state.page=totalPages; sel.value=String(state.page);}else{ if(state.page>totalPages)state.page=totalPages;}
...
function renderFNGCard(data){
  try{const item=Array.isArray(data?.data)?data.data[0]:null; if(!item){setHTML("#fng-title","- / -"); setHTML("#fng-gauge",""); return;}
    ...
    const pct=clamp(value/100,0,1), start=-Math.PI, end=0, ang=start+(end-start)*pct, r=56, cx=80, cy=86;
...
/* Mini caps */
function renderMiniCaps(btc,usdt){const toArr=d=>Array.isArray(d?.market_caps)?d.market_caps.map(x=>x[1]):null; const a=toArr(btc), b=toArr(usdt); setHTML("#kpi-btc-spark", a ? sparklineSVG(a,180,44) : ""); setHTML("#kpi-usdt-spark", b ? sparklineSVG(b,180,44) : "");}
...
function renderLongShort(period,arr){const last=Array.isArray(arr)?arr.at(-1):null; if(!last){setHTML("#ls-long","-"); setHTML("#ls-short","-"); setHTML("#ls-ratio","-"); return;}
  ...
  setHTML("#ls-long",`${longPct.toFixed(1)}%`); setHTML("#ls-short",`${shortPct.toFixed(1)}%`); setHTML("#ls-ratio", ratio?ratio.toFixed(2):`${(longPct/shortPct).toFixed(2)}`); $("#ls-longbar").style.width=`${clamp(longPct,0,100)}%`; $$('.ls-ctl button').forEach(b=>b.classList.toggle('active', b.dataset.period===period));
}
...
async function initData(){
  try{
    ...
    applySortFilter(); requestAnimationFrame(applySortFilter); // 초기 표시 누락 방지
    renderFNGCard(fng);
